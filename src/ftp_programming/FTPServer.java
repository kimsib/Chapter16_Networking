...
/**
 * 파일 서비스 프로그래밍 1. 클라이언트는 특정한 파일명을 서버에 요청한다. 2. 서버는 클라이어느가 요청한 파일을 찾는다. 3. 요청한
 * 파일을 찾으면 파일(리소스폴더)에서 데이터를 읽어 들인다. 4. 읽어 온 파일의 데이터를 클라이언트에게 전송한다. 5. 클라이언트는 받은
...
public class FTPServer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 단계2 : 파일 데이터를 찾아내고 데이터 읽는다
			fin = new FileInputStream("resources\\" + fileName);
			
			// 단계3 : 파일데이터를 읽고 데이터를 보낸다.
